...
const Card:React.FC<props> = ({header,small,features,name, onPressed,color,textColor}) => {
    ...
    return (
        <div className="p-4 my-2 mr-5 flex justify-start flex-col shadow-2xl gap-0 w-44 rounded-xl bg-white hover:shadow-slate-400"
        style={{
            background:color
        }}
        >
            ...
            <div className="flex flex-col gap-0">
                <h2 className="relative ml-2"
                style={{
                    color: textColor
                }}
                >{`${name}`}</h2>
                <p className="text-xxs relative ml-2 opacity-50"
                style={{
                    color: textColor
                }}
                >{`${small}`}</p>
            ...
            <br></br>
            <button  className="p-3 w-auto text-center bg-slate-900 text-white rounded-lg border-none hover:bg-gray-500 shadow-sm cursor-pointer" onClick={onPressed}
            style={{
                color: textColor
            }}
            >
